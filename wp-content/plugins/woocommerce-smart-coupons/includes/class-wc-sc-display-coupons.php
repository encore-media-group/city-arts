...
if ( ! class_exists( 'WC_SC_Display_Coupons' ) ) {
	...
	class WC_SC_Display_Coupons {
		...
		public function show_available_coupons( $available_coupons_heading = '', $page = 'checkout' ) {
			...
			<div id="coupons_list" style="display: none;"><h3><?php echo __( stripslashes( $available_coupons_heading ), 'woocommerce-smart-coupons' ); // phpcs:ignore ?></h3><div id="all_coupon_container">
				...
				foreach ( $coupons as $code ) {
					...
					if ( ( empty( $coupon_amount ) ) && ( ( 'yes' !== $is_free_shipping ) || ( ! empty( $discount_type ) && ! in_array( $discount_type, array( 'free_gift', 'smart_coupon' ), true ) ) ) ) {
		...
		public function sc_endpoint_content() {
			...
			<p><?php echo esc_html__( 'List of coupons which are valid & available for use. Click on the coupon to use it. The coupon discount will be visible only when atleast one product is present in the cart.', 'woocommerce-smart-coupons' ); ?></p>
